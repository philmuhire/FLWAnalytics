...
public interface FoodProcessRepo extends JpaRepository<FoodProcess, Long> {
    ...
    @Query("select new com.phil.flwanalytics.analytics.projection.FoodProduce(f.food, f.country, f.produce, f.lossQuantity, f.year) from FoodProcess as f group by f.food, f.country, f.year")
    List<FoodProduce> getProduceOfAllYears();

    @Query("select new com.phil.flwanalytics.analytics.projection.FoodProduce(f.food, f.country, f.produce, f.lossQuantity, f.year) from FoodProcess as f where f.food.id=?1 group by f.food, f.country, f.year ")
