...
public class ProcessCtrl {
    ...
    public ResponseEntity<?>saveActivity(@RequestBody ProcessPayload ap) {
        ...
        Process process;
        URI uri = URI.create(ServletUriComponentsBuilder.fromCurrentContextPath().path("/api/process/save").toUriString());
        try {
            log.info("trying in controller");
            log.info(ap.toString());
            process = processService.saveActivity(ap);
    ...
    public ResponseEntity<?>getActivity(@PathVariable Long id) {
        Process process;
        URI uri = URI.create(ServletUriComponentsBuilder.fromCurrentContextPath().path("/api/process/getone").toUriString());
        try {
