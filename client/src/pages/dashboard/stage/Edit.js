...
import { addNewStage, editStage, fetchStages, getCurrentStage, getStageError, getStageStatus } from '../../../services/reducers/stageSlice';
import { validateCrop, validateStage } from '../../../utils/functions';

const Edit = ({ toggleEditStgMdl, setToggleEditStgMdl }) => {
    const dispatch = useDispatch();
    const [submit, setSubmit] = useState(false)
    const [formErrors, setFormErrors] = useState({ 1: 1 })

    const current = useSelector(getCurrentStage);
    ...
    useEffect(() => {
        if (status === "succeeded-editstage") {
            console.log("got here")
            document.getElementById("editstage").reset()
            setToggleEditStgMdl(false)
    ...
    return (
        <div className={`fixed flex items-center align-center bg-gray-500 bg-opacity-50 z-50 show w-full md:inset-0 md:h-full ${toggleEditStgMdl ? "" : " hidden"}`}>
            <div className="relative w-full h-full flex justify-center p-4 items-center md:h-screen">
                <div className="relative bg-white rounded-lg w-1/4 shadow dark:bg-gray-700">
                    <div className="flex items-center justify-between p-5 border-b rounded-t dark:border-gray-600">
                        ...
                        <button type="button" className="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center dark:hover:bg-gray-600 dark:hover:text-white" onClick={() => setToggleEditStgMdl(false)}>
                    ...
                    <form id='editstage' onSubmit={handleSubmit} className='flex  flex-col items-center py-3'>
                        <div className="relative z-0 w-4/5 mb-3 group">
                            <input value={stageState.name} type="text" name="name" onChange={(e) => { handleChange(e) }}
                        ...
                        <div className="relative z-0 w-4/5 mb-3 group">
                            <input value={stageState.description} type="text" name="description" onChange={(e) => { handleChange(e) }}
                                className="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer"
                                placeholder=" " required="" autoComplete='off' />
                            <label htmlFor="name"
                                className="peer-focus:font-medium absolute text-sm text-gray-500 dark:text-gray-400 duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:left-0 peer-focus:text-blue-600 peer-focus:dark:text-blue-500 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6">
