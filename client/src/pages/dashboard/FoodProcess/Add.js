...
import { addNewProcess, fetchactivities, getActError, getActStatus, selectAllActivities } from '../../../services/reducers/activitySlice';
import { fetchCountries, selectAllCountries } from '../../../services/reducers/countrySlice';
import { fetchAllCrops, fetchFirst, getAllCrops } from '../../../services/reducers/cropSlice';
import { addNewFoodProcess, fetchNProduce, getFPError, getFPStatus } from '../../../services/reducers/foodProcessSlice';
import { fetchStages, selectAllStages } from '../../../services/reducers/stageSlice';
import { validateFP, validateProcess } from '../../../utils/functions';

const Add = ({ togAddFPMdl, setTogAddFPMdl }) => {
    ...
    const handleChange = (e) => {
        ...
        setfpState({ ...fpState, [name]: value })
    };
    const handleSubmit = () => {
        setFormErrors(validateFP(fpState));
    }
    ...
    useEffect(() => {
        if (submit) {
            ...
            try {
                dispatch(addNewFoodProcess(fpState));
            } catch (err) {
                console.log(err)
            }
        }
    ...
    useEffect(() => {
        if (status === "succeeded-addfp") {
            console.log("got here")
            document.getElementById("savefp").reset()
            setTogAddFPMdl(false)
    ...
    return (
        <div className={`fixed flex items-center align-center bg-gray-500 bg-opacity-50 z-50 show w-full md:inset-0 md:h-full ${togAddFPMdl ? "" : " hidden"}`}>
            <div className="relative w-full h-full flex justify-center p-4 items-center md:h-screen">
                <div className="relative bg-white rounded-lg w-1/4 shadow dark:bg-gray-700">
                    <div className="flex items-center justify-between p-5 border-b rounded-t dark:border-gray-600">
                        ...
                        <button type="button" className="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center dark:hover:bg-gray-600 dark:hover:text-white" onClick={() => setTogAddFPMdl(false)}>
                    ...
                    <form id='savefp' onSubmit={handleSubmit} className='flex  flex-col items-center py-3'>
                        ...
                        <div className="relative z-0 w-4/5 mb-3 group">
                            ...
                            <p className="text-xs text-red-500">{formErrors.foodId}</p>
                        </div>

                        <div className="relative z-0 w-4/5 mb-3 group">
                            <select name="countryName" onChange={(e) => { handleChange(e) }}
                                className="block text-sm pl-2 py-2.5 px-0 w-full text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer"
                                placeholder=" " required="" autoComplete='off' >
                                <option className='text-xs' value="">Select Country</option>
                                {
                                    countries.length > 0 ? (
                            ...
                            <p className="text-xs text-red-500">{formErrors.countryName}</p>
                        </div>
                        <div className="relative z-0 w-4/5 mb-3 group">
                            <input type="text" name="produce" onChange={(e) => { handleChange(e) }}
                            ...
                            <p className="text-xs text-red-500">{formErrors.produce}</p>

                        </div>
                        <div className="relative z-0 w-4/5 mb-3 group">
                            <input type="text" name="lossQuantity" onChange={(e) => { handleChange(e) }}
                                className="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer"
                                placeholder=" " required="" autoComplete='off' />
                            <label htmlFor="name"
                                className="peer-focus:font-medium absolute text-sm text-gray-500 dark:text-gray-400 duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:left-0 peer-focus:text-blue-600 peer-focus:dark:text-blue-500 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6">
                            ...
                            <p className="text-xs text-red-500">{formErrors.lossQuantity}</p>

                        </div>
                        <div className="relative z-0 w-4/5 mb-3 group">
                            <input type="text" name="lossPercentage" onChange={(e) => { handleChange(e) }}
                                className="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer"
                                placeholder=" " required="" autoComplete='off' />
                            <label htmlFor="name"
                                className="peer-focus:font-medium absolute text-sm text-gray-500 dark:text-gray-400 duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:left-0 peer-focus:text-blue-600 peer-focus:dark:text-blue-500 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6">
                            ...
                            <p className="text-xs text-red-500">{formErrors.lossPercentage}</p>
                        ...
                        <div className="relative z-0 w-4/5 mb-3 group">
                            ...
                            <p className="text-xs text-red-500">{formErrors.year}</p>

                        </div>
                        <div className="relative z-0 w-4/5 mb-3 group">
                            <select name="processId" onChange={(e) => { handleChange(e) }}
                                className="block text-sm pl-2 py-2.5 px-0 w-full text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer"
                                placeholder=" " required="" autoComplete='off' >
                            ...
                            <p className="text-xs text-red-500">{formErrors.processId}</p>
                        </div>
